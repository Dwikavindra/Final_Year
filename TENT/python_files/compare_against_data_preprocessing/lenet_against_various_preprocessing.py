...
def predict_image(model, x, y):
    with torch.no_grad():
        ...
        for i in range(len(pred)):
            heuristic_pred = apply_heuristic(pred[i])
            result.append({
...
if __name__ == "__main__":
    ...
    elif args.transform == "noprocessing":
        dataset = CustomImageDataset('../workspace/dwika/fyp/data_election/batch_inference.csv', '', transform=clean_data.image_processing_no_effect)
    elif args.transform == "otsu_threshold":
        dataset = CustomImageDataset('../workspace/dwika/fyp/data_election/batch_inference.csv', '', transform=clean_data.transform_image_otsu_only)

    dataLoader = DataLoader(dataset, batch_size=512, shuffle=True)

    model = torch.load("/workspace/Dwika/fyp/save_model.pt", map_location=torch.device("cuda"), weights_only=False)
    model.eval()

    raw_result = infer_by_batch(model, dataLoader)
    accuracy, precision, recall, f1 = get_result(raw_result)
