...
if __name__ == "__main__":
    ...
    result=sirekap_helper.evaluate_ensemble_models(models,dataLoader)
    sirekap_helper.write_row_to_csv(
        file_path=f"{args.result_path}/confirm_mnist_sirekap_ensemble.csv", #
        columns=["model_name", "iteration", "accuracy"],
        values=["ensembled",args.iteration, result["accuracy"]]
    )
